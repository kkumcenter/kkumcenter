...
| post-attachments | 비공개 | 첨부파일. 필요 시 다운로드 권한 확인 |

## 16. 실제 적용 전 확인할 점

SQL을 Supabase에 실행하기 전에 아래를 확인해야 합니다.

- 관리자 계정 2명을 어떤 이메일로 만들지
- 회원가입에서 연락처 중복을 허용할지
- 비회원 조회 비밀번호를 어디에서 암호화할지
- 공간예약 중복 시간 검사를 서버에서 처리할지
- 프로그램 정원 초과와 대기자 처리를 어디에서 처리할지
- 첨부파일을 공개 다운로드로 둘지, 권한 확인 후 다운로드로 둘지
