...
- `program-save`: 관리자 전용 교육 등록·수정 요청입니다. 교육은 기록 보존을 위해 공개 상태로 저장하고, 노출은 모집상태와 수업상태로 관리합니다.
- `program-hide`: 이전 숨김 호출과의 호환용입니다. 실제 숨김 대신 `접수마감` 상태로 전환합니다.
- `program-restore`: 이전 복구 호출과의 호환용입니다. 교육을 공개 상태로 유지합니다.
