...
- `program-save`: 관리자 전용 교육 등록·수정 요청입니다. `isActive` 값으로 공개/숨김 상태도 함께 저장합니다.
- `program-hide`: 관리자 전용 교육 숨김 처리입니다. 화면에서는 공개상태 저장 방식으로 통합했으며, 기존 호출 호환용으로 유지합니다.
- `program-restore`: 관리자 전용 교육 복구 처리입니다. 화면에서는 공개상태 저장 방식으로 통합했으며, 기존 호출 호환용으로 유지합니다.
