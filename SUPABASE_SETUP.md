...
Supabase 정보가 비어 있거나 비회원 신청 암호화 함수가 아직 연결되지 않은 동안에는 기존 목업처럼 브라우저 임시 저장 방식으로 작동합니다.
...
현재 화면 코드는 로그인한 회원과 관리자 DB 연결을 우선 준비해 둔 상태이며, 비회원 신청의 실제 DB 저장은 암호화 함수 연결 후 활성화하는 순서가 안전합니다.
...
1. 관리자 로그인 확인
2. 공간예약 신청과 관리자 승인 흐름 검증
3. 교육신청 선착순 접수와 대기자 흐름 검증
4. 문의/제안 작성과 답변 연결
5. 공지사항, 마을 이야기, 갤러리 관리 연결
6. 관리자 페이지 화면 세분화
